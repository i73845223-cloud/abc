...
export async function GET(req: Request) {
  try {
    ...
    if (!currency) {
      return NextResponse.json({ minAmountUsd: 5 });
    }
    ...
    if (cached && cached.expires > Date.now()) {
      ...
      return NextResponse.json({ minAmountUsd: cached.value });
    }
    ...
    if (minRes.status === 404) {
      console.warn(`[MIN-AMOUNT] Currency ${normalized} not supported, using fallback 5 USD`);
      cache.set(cacheKey, { value: 5, expires: Date.now() + 60 * 60 * 1000 });
      return NextResponse.json({ minAmountUsd: 5 });
    }
    ...
    const roundedMinUsd = Math.ceil(minUsd * 100) / 100;

    cache.set(cacheKey, { value: roundedMinUsd, expires: Date.now() + 10 * 60 * 1000 });

    return NextResponse.json({ minAmountUsd: roundedMinUsd });
  } catch (error) {
