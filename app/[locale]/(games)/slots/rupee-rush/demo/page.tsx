...
import slotBg from "@/public/rupee-rush/bg.webp";
import frameOverlay from "@/public/rupee-rush/frame.webp";
...
const SlotMachine = () => {
  ...
  const checkWin = useCallback((reels: typeof SYMBOLS[0]['symbol'][][], bet: number, isMuted: boolean): { 
    ...
    const animateReels = () => {
      ...
      const symbolHeight = isXS ? 62 : isMobile ? 84 : 140;
  ...
  const renderSymbol = (symbol: typeof SYMBOLS[0]['symbol'], colIndex: number, rowIndex: number) => {
    ...
    return (
      <div 
        className={`flex justify-center items-center w-full sm:h-[84px] xl:h-[140px] h-[62px] transition-all duration-300 ${
  ...
  const renderReel = (col: typeof SYMBOLS[0]['symbol'][], colIndex: number) => (
    <div 
      key={colIndex} 
      className="flex flex-col items-center h-full w-[70px] sm:w-[95px] xl:w-[160px] overflow-hidden relative mx-[3.5px] z-10"
    >
      <div 
  ...
  return (
    <div className="bg-black overflow-hidden min-h-screen min-w-screen relative">
      ...
      <div className="flex flex-col items-center xl:justify-center gap-8 text-center min-h-[calc(100vh-100px)] relative z-2">
        <div className="absolute xl:mt-[100px] sm:mt-[200px] mt-[160px] [@media(max-height:700px)]:mt-[110px] xl:left-[150px] md:left-[50px] z-3">
          <div className="relative flex justify-center items-center w-[95vw] h-[205px] sm:h-[270px] xl:w-[750px] max-w-[340px] sm:max-w-[450px] xl:max-w-[750px] xl:h-[440px] overflow-hidden p-2 rounded-xl">
            {reels.map((col, colIndex) => renderReel(col, colIndex))}
          </div>
          
          <div className="absolute inset-0 w-full h-full z-1 xl:top-[-15px] top-[0px] sm:top-0 -translate-y-1/2 sm:max-w-[450px] xl:max-w-[750px]">
            <Image 
              width={750}
              src={frameOverlay} 
              alt="Frame Overlay" 
