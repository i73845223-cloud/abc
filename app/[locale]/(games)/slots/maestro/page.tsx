...
import slotBg from "@/public/maestro/bg.webp";
import frameOverlay from "@/public/maestro/frame.webp";
...
const SlotMachine = () => {
  ...
  const checkWin = useCallback((reels: typeof SYMBOLS[0]['symbol'][][], bet: number, isMuted: boolean): { 
    payout: number; 
...
const spinReels = useCallback(async () => {
    ...
    try {
      ...
      const finalReels = Array(3).fill(null).map(() => Array(3).fill(null));
  ...
  const animateReels = () => {
    ...
    const symbolHeight = isXS ? 64 : isMobile ? 90 : 145;
  ...
  const renderSymbol = (symbol: typeof SYMBOLS[0]['symbol'], colIndex: number, rowIndex: number) => {
    ...
    return (
      <div 
        className={`flex justify-center items-center w-full sm:h-[90px] xl:h-[145px] h-[64px] transition-all duration-300 ${
  ...
  const renderReel = (col: typeof SYMBOLS[0]['symbol'][], colIndex: number) => (
    <div 
      key={colIndex} 
      className="flex flex-col items-center h-full w-[80px] sm:w-[110px] xl:w-[185px] overflow-hidden relative xl:mx-[5px] sm:mx-[6px] mx-[4px] z-10"
    >
      <div 
        ref={el => { reelRefs.current[colIndex] = el; }}
        className="flex flex-col absolute xl:top-[1px] gap-[1px] w-full"
      >
        {[...Array(15)].map((_, i) => {
          const rowIndex = i % 3;
  ...
  return (
    <div className="bg-black overflow-hidden min-h-screen min-w-screen relative">
      ...
      <div className="flex flex-col items-center xl:justify-center gap-8 text-center min-h-[calc(100vh-100px)] relative z-2">
        <div className="absolute xl:mt-[10px] sm:mt-[200px] mt-[160px] [@media(max-height:700px)]:mt-[110px] xl:left-[150px] md:left-[50px] z-3">
          <div className="relative flex justify-center items-center w-[95vw] h-[210px] sm:h-[290px] xl:w-[760px] max-w-[340px] sm:max-w-[470px] xl:max-w-[760px] xl:h-[455px] overflow-hidden p-2 rounded-xl">
            {reels.map((col, colIndex) => renderReel(col, colIndex))}
          </div>
          
          <div className="absolute inset-0 w-full h-full z-1 xl:top-[65px] top-[35px] sm:top-[45px] -translate-y-1/2 sm:max-w-[470px] xl:max-w-[760px]">
            <Image 
              width={760}
              src={frameOverlay} 
              alt="Frame Overlay" 
        ...
        <button
          ...
          className={`absolute bottom-0 mb-[40px] xl:mb-0 md:inset-y-auto flex items-center justify-center xl:w-[220px] w-[150px] xl:h-56 border-2 border-white rounded-full transition-all duration-300 xl:right-[150px] md:right-[50px] xl:mt-0 md:mt-[200px] cursor-pointer z-20 ${
