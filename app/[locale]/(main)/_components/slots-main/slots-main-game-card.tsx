  ...
  return (
    <div className="relative group rounded-xl overflow-hidden">
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-xl border-2 border-red-600">
        <h3 className="absolute top-3 left-4 text-white text-md font-bold">
          {game.name}
        </h3>
        <Link
          href={game.href}
          className="flex justify-center items-center p-8 bg-red-600 rounded-full hover:bg-red-700 group/play transition-colors relative scale-125"
        >
          <svg
        ...
        <Link
          href={game.demoHref}
          className="text-white rounded-md transition-colors text-center hover:text-red-600 mt-2 absolute bottom-3 text-md"
        >
          {t('demo')}
        </Link>
      </div>
